...
## Introducción
```
From the point of view of a company that has decided to use Stream Hatchet's BI I think
it would be useful to add a section where users could check easily which sponsors each 
streamer has, as well as the sponsors that participate in different tournaments.
...
recently been gaining popularity and would be a more economical target to be approached by sponsors.
We could show this with a line graphic or an histogram.
Once we have these data we could show a pie chart with the companies that invest more in each sectors.
```
